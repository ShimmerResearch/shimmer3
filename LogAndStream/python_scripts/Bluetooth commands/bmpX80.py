...
if len(sys.argv) < 2:
    ...
    print(
        "Packet Type \tTimestamp diff \tUncompensated Temperature \tTemperature (øC) \tUncompensated Pressure \tPressure (Pa)")
    try:
        while True:
            while numbytes < framesize:
                ddata += ser.read(framesize)
            ...
            (UT_msb, UT_lsb, UP_msb, UP_lsb, UP_xlsb) = struct.unpack('BBBBB', data[4:framesize])
