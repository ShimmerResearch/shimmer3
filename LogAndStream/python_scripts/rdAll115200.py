...
# read incoming data
   ...
   print "---------------------------------------- mac: " 
   inArg = [0x24, 0x03, 0x02, UART_COMP_SHIMMER, UART_PROP_MAC]
   sendUart(inArg) 
   print "---------------------------------------- Shimmer ver: "
   inArg = [0x24, 0x03, 0x02, UART_COMP_SHIMMER, UART_PROP_VER]
   sendUart(inArg)
   print "---------------------------------------- Bluetooth ver: "
   inArg = [0x24, 0x03, 0x02, UART_COMP_BT, UART_PROP_VER]
   sendUart(inArg)
   print "---------------------------------------- bat: "
   inArg = [0x24, 0x03, 0x02, UART_COMP_BAT, UART_PROP_VALUE]
   sendUart(inArg)   
   print "---------------------------------------- cfg time read: " 
   inArg = [0x24, 0x03, 0x02, UART_COMP_SHIMMER, UART_PROP_RWC_CFG_TIME]
   sendUart(inArg)   
   print "---------------------------------------- cfg time wr: " 
   inArg = [0x24, 0x01, 0x0a, UART_COMP_SHIMMER, UART_PROP_RWC_CFG_TIME, 0x00, 0x11, 0x22, 0x33, 0x44, 0x55, 0x66, 0x77]
   sendUart(inArg)   
   print "---------------------------------------- cfg time read again: " 
   inArg = [0x24, 0x03, 0x02, UART_COMP_SHIMMER, UART_PROP_RWC_CFG_TIME]
   sendUart(inArg)   
   print "---------------------------------------- curr time read: " 
   inArg = [0x24, 0x03, 0x02, UART_COMP_SHIMMER, UART_PROP_CURR_LOCAL_TIME]
   sendUart(inArg)   
   print "---------------------------------------- Daughter Card ID get: " 
   inArg = [0x24, 0x03, 0x04, UART_COMP_DAUGHTER_CARD, UART_PROP_CARD_ID, 0x10, 0x00]
   sendUart(inArg)  
   print "---------------------------------------- Daughter Card Mem get: " 
   inArg = [0x24, 0x03, 0x05, UART_COMP_DAUGHTER_CARD, UART_PROP_CARD_MEM, 0x80, 0x00, 0x00]
   sendUart(inArg)  
   print "---------------------------------------- info Mem D get: " 
   inArg = [0x24, 0x03, 0x05, UART_COMP_SHIMMER, UART_PROP_INFOMEM, 0x80, 0x00, 0x00]
   sendUart(inArg) 
   print "---------------------------------------- info Mem C get: " 
   inArg = [0x24, 0x03, 0x05, UART_COMP_SHIMMER, UART_PROP_INFOMEM, 0x80, 0x80, 0x00]
   sendUart(inArg) 
   print "---------------------------------------- info Mem B get: " 
   inArg = [0x24, 0x03, 0x05, UART_COMP_SHIMMER, UART_PROP_INFOMEM, 0x80, 0x00, 0x01]
   sendUart(inArg) 
