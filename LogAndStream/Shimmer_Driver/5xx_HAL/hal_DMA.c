...
void DMA0_init(uint16_t *srcAddr, uint16_t *destAddr, uint16_t size)
{
  ...
  DMA0CTL = DMADT_1 + DMADSTINCR_3 + DMASRCINCR_3 + DMAEN
      + DMAIE;   //block transfer, inc dst, inc src, Int enable
...
void DMA1_init(uint8_t *srcAddr, uint8_t *destAddr, uint16_t size)
{
  ...
  DMACTL4 = DMARMWDIS;    //Read-modify-write disable
  DMA1CTL &= ~DMAIFG;
  DMA1CTL = DMADT_0 + DMADSTINCR_0 + DMASRCINCR_3 + DMAIE + DMASBDB
      + DMALEVEL; //block transfer, fixed dst, inc src, Int enable//
