...
elif len(sys.argv) < 6 and len(sys.argv) != 2:
   print "A second, third, fourth and fifth argument must be specified to change the configured settings."
   print "Example:"
   print "   setConfigBytes.py Com12 0x11 0x22 0x33 0x44"
   print "or"
...
# check if new config bytes values are valid
      ...
      if (0 <= newconfig0 <=255) and (0 <= newconfig1 <=255) and (0 <= newconfig2 <=255) and (0 <= newconfig3 <= 255): 
         ser = serial.Serial(sys.argv[1], 115200)
         ser.flushInput()
# send new config setup bytes values 
         ser.write(struct.pack('BBBBB', 0x0E, newconfig0, newconfig1, newconfig2, newconfig3)) #SET_CONFIG_SETUP_BYTES_COMMAND followed by new config setup byte values
         wait_for_ack()
         print "Configuration bytes set to: 0x%02x 0x%02x 0x%02x 0x%02x." % (newconfig0, newconfig1, newconfig2, newconfig3)
#close serial port
         ser.close()
      else:
         print "0x%02x 0x%02x 0x%02x 0x%02x is not a valid configuration." % (newconfig0, newconfig1, newconfig2, newconfig3)
print "All done!"
