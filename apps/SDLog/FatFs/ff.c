...
)
{
	...
	volatile uint32_t time_in, time_out, td;
	volatile UINT b = btw;

	*bw = 0;	/* Initialize byte counter */


	time_in = ((uint32_t)RTCTIM1<<16)+RTCTIM0;
	res = validate(fp->fs, fp->id);			/* Check validity */
	if (res != FR_OK) LEAVE_FF(fp->fs, res);
	if (fp->flag & FA__ERROR)				/* Aborted file? */
		LEAVE_FF(fp->fs, FR_INT_ERR);
	...
	time_out = ((uint32_t)RTCTIM1<<16)+RTCTIM0;
	td = time_out - time_in;
	_NOP();
	LEAVE_FF(fp->fs, FR_OK);
}
...
void set_sd_detect(unsigned char detect){
	run_mmc_detectsd(detect);
}
#endif /* !_FS_READONLY */
#endif /* _USE_STRFUNC */
