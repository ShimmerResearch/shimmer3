...
//--#include "USB_eMPL/usbConstructs.h"
...
extern unsigned char check_uart_mode_enabled;
...
#define min(a,b) ((a < b) ? a : b)
       ...
       // cdcSendDataWaitTilDone((BYTE*)out, PACKET_LENGTH, CDC0_INTFNUM, 0);
        if(check_uart_mode_enabled){
//--        		while(!msp430_uart_tx(out, PACKET_LENGTH));
//--MNs        	while(!msp430_uart_tx(PACKET_LENGTH, out));
        		msp430_uart_tx(PACKET_LENGTH, out);
        	}else{
...
/*
	out[0] = (unsigned char) (quat[0] >> 24);
    out[1] = (unsigned char) ((uint16_t)(quat[0] >> 16) % 256);
    out[2] = (unsigned char) (quat[1] >> 24);
    out[3] = (unsigned char) ((uint16_t)(quat[1] >> 16) % 256);
    out[4] = (unsigned char) (quat[2] >> 24);
    out[5] = (unsigned char) ((uint16_t)(quat[2] >> 16) % 256);
    out[6] = (unsigned char) (quat[3] >> 24);
    out[7] = (unsigned char) ((uint16_t)(quat[3] >> 16) % 256);
    out[8] = 'q';
    out[9] = packet_count++;
*/
#endif
